...
colnames(data)=c("Gene","Mut","Mean_score","Value","Simulations","Pval")
data=data[ order(data[,6], data[,1]), ]
...
a=legend("topright",inset=c(-0.45,0),legend = data2$Gene, xpd=TRUE, bty = 'n',text.col=unlist(data2$color))
garbage <- dev.off()
...
grid.text(round(fisher.test(matrix(c(length(a1)-n12-n13,n12,n13,n123),2,2))$p.value,4))
garbage <- dev.off()
...
integrated_method = "ExInAtor3" # When p-values are identical for several genes, their order in this dataset is used

# Loading the file and calculating q-values
pvals = as.matrix(data$Pval)
colnames(pvals)=c("ExInAtor3")
genelist = data$Gene
