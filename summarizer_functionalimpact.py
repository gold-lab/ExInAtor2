		...
		for file in os.listdir(path+sample+"/"):
			if str(simulations)+"_12" in file:
				...
				for line in file:
					...
					all_gene_list[sample][gene]=float(line.split(" ")[6])
					all_gene_list2[sample][gene]=float(line.split(" ")[5])
...
out_file =  open (path+"all_genes_combined_"+simulations+"_12.txt","w")
...
out_file =  open (path+"all_genes_combined_"+simulations+"_12_pvalue.txt","w")
...
out_file1 =  open (path+"summary_per_gene_"+simulations+"_12.txt","w")
...
for gene in sorted(total_counter, key=total_counter.get, reverse=True):
...
out_file2 =  open (path+"summary_per_sample_"+simulations+"_12.txt","w")
out_file2.write("Sample\tQ-value_<=0.1\tQ-value_<=0.01\tQ-value_<=0.001\tQ-value_<=0.0001\n")

for sample in sorted(percentage_counter, key=percentage_counter.get, reverse=True):
